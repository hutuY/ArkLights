...
- [禁止开启速通输入法](#脚本卡在输入法设置界面)，否则脚本重启卡住。
...
| 模拟器           | 使用前提/安卓版本/问题                                       |
| ---------------- | ------------------------------------------------------------ |
| 雷电4            | 若出现颠倒尝试[模拟器屏幕颠倒/旋转/竖屏](#模拟器屏幕颠倒/旋转/竖屏) |
| 逍遥             | 设置中勾选OpenGL渲染                                         |
| 夜神             | 模拟器版本>=7.0.2.2                                          |
| 蓝叠             | 设置中勾选高级/高阶/兼容图形引擎 长时间使用执行下文命令      |
| mumu             | mumu必须是mumu9 但是启动闪屏                                 |
| genymotion       | 安卓10                                                       |
| redroid          | 安卓10                                                       |
| Android Emulator |                                                              |
...
adb -s 127.0.0.1:5555 shell "nohup sh -c 'nc -Lp49876 sh' > /dev/null 2>&1 &"
```

#### 云手机

支持 红手指（默认安卓6需要找客服升级8.1）、雷电云、华云、多多云、合鑫云、双子星、云帅云

不支持 移动云

<!-- 蘑菇云需勾`禁用完全重启`，并自行定期强制停止脚本 -->

<!-- 注意： -->
<!-- 1. 从群友反馈看，红手指kvip、雷电云svip/bvip跑速通稳定性好。 -->
...
#### 怎么自动开无障碍/怎么每次都需要打开无障碍

有root就会自动开无障碍。软件启动后的无障碍提示选“以后不再提醒”，进入后直接点“确定”。
被系统杀后台后必须重新开启 尝试虚拟应用或root
...
[图片教程]https://s3.bmp.ovh/imgs/2022/07/08/f9e2a00d819f8160.png) 
...
勾选`轮次作战`，作战设置填`上一次  `如果无效填写缩写`syc`

#### 为什么不刷活动关？为什么不刷关?

| 可能性                     | 解决方法                            |
| -------------------------- | :---------------------------------- |
| 活动更新后短期内作者没更新 | 等待更新或使用[上一次](#怎么只刷上一次)                |
| 设备分辨率不支持    | 更换设备或查看[手机真机](#手机真机) |
| 脚本设置错误               | [设备兼容](#设备兼容性)             |
...
直接填写关卡名 或 勾选`轮次作战`，作战列表填写 `长期委托X`（X是数字，按每个号的情况来设，比如`长期委托2`）剿灭列表中的第2个关卡，启动。长期委托全过的，可直接用关卡名。
...
<!-- #### 作战出现非三星代理/代理失误时会跳过吗 -->
<!---->
<!-- 出现时通知QQ，连续出现三次后跳过当前关，在高级设置中设置跳过次数。 -->
...
设置常见近卫技能，如(数字表示技能)耀光1、煌2、山2、帕拉斯1、赫拉格2、史尔特尔2、银灰1、羽毛笔1、幽灵鲨1、拉狗2。
...
是。选任一精一近卫刷。不妨和其他脚本对比下效率。

#### 肉鸽什么干员最好？

![干员测试](https://s3.bmp.ovh/imgs/2022/07/08/0e5bc874d0b9aec7.png)
...
一段时间内多次登陆时出现，会自动解锁。出现后会通过通知方式通知
...
> 小: 请问一直在主界面是什么原因呀，手动点进其他界面也会自动回到主界面
> Li: 建议多看必读
> 小: 好的，我再研究一下必读（
> 小: 这样吗，好像还是会一直回到主界面
> 葫: 关掉游戏模式
> 小: 啊好像好了，可能是这个原因
	（虽然昨天关了游戏模式也运行不了，不知道为什么  
